...
{
    if (group.GetFirstBotMember() == nullptr) return;
...
void BotChatHandler::loadConfig()
{
    ...
    _minBaseDelay = sConfigMgr->GetOption<int32>("NpcBot.Chat.Delay.minBase", 750);
    _perCharDelay = sConfigMgr->GetOption<int32>("NpcBot.Chat.Delay.perChar", 20);
